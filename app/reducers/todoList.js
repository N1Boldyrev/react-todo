        ...
        case "SET_ACTIVE": {
            ...
            for (let key in state.tasks) {
                if (
                    ...
                    state.tasks[key].id == state.nowActive &&
                    state.nowActive != action.payload
                ) {
                    newObj.tasks[key].className = "listElement";
                }
                if (state.tasks[key].id == action.payload) {
                    //Делаем нажатый элемент активным
                    newObj.tasks[key].className = "listElement active";
                    newObj.nowActive = action.payload;
                    newObj.activeTitle = state.tasks[key].taskName;
        ...
        case "CHANGE_ELEM_NAME": {
            ...
            for (let key in state.tasks) {
                if (state.tasks[key].id == action.payload.id) {
                    newObj.tasks[key].taskName = action.payload.newName;
                    newObj.activeTitle = action.payload.newName;
                    return newObj;
                }
            }
        }
